...
| `src/jarvis/llm/llm.spec.md` | Pluggable LLM backend abstraction: `LLMBackend` ABC, `OllamaBackend` (only impl in PR 1), factory, function-style helpers | Provider-agnostic interface so Jarvis can run on Ollama, OpenAI-compatible (LM Studio / oMLX), or Anthropic-compatible servers; phased rollout, zero behaviour change in PR 1 |

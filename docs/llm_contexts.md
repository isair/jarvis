...
> **Backend abstraction.** Every context below routes through `jarvis.llm` ([spec](../src/jarvis/llm/llm.spec.md)). The reply hot path (engine, planner, evaluator, enrichment, graph_ops, tool selection, weather + logMeal extractors) is migrated to `get_llm_backend(cfg)` factory dispatch — picking `llm_provider: openai_compatible` swaps the wire shape without touching call sites. **Model resolution for migrated contexts** goes through `resolve_chat_model(cfg)` — `cfg.llm_chat_model` first, falling back to `cfg.ollama_chat_model`. Where this doc says "the active chat model" or "resolved chat model" below, that is the chain it refers to. Diary maintenance (`memory/conversation.py`) and the intent judge (`listening/intent_judge.py`) still call the Ollama-only paths directly and read `cfg.ollama_*` literally; PR 2.5b / 2.5c will migrate them.
...
- **Model / gating**: factory-dispatched via `get_llm_backend(cfg)` — resolved chat model (`resolve_chat_model(cfg)`: `llm_chat_model → ollama_chat_model`). Not optional. No size branching on the loop itself — size branching affects the digests/evaluator around it.
- **Inputs**:
  - Redacted user query
...
- **Model / gating**: resolved via `resolve_tool_router_model(cfg)` — `tool_router_model → intent_judge_model → resolve_chat_model(cfg)`. Factory-dispatched. Small classification task; rides the same small/warm model as the router. Silent empty-dict on failure (early-return when no chat model is configured — no wasted LLM round-trip).
...
- **Model / gating**: resolved chat model (factory-dispatched). Gated by `memory_digest_enabled`. Auto-on detection reads the same resolved chat model so OpenAI-compatible deployments with a small `llm_chat_model` enable the digest correctly even when the legacy `ollama_chat_model` alias still names a large model.
...
- **Model / gating**: resolved chat model (factory-dispatched). Gated by `tool_result_digest_enabled` — auto-on for SMALL via `detect_model_size(resolve_chat_model(cfg))`.
...
- **Model / gating**: `_resolve_loop_digest_model(cfg)` — prefers `intent_judge_model`, falls back to the resolved chat model. Factory-dispatched.
...
- **Model / gating**: `resolve_tool_router_model(cfg)` chain — `tool_router_model → intent_judge_model → resolve_chat_model(cfg)`. Factory-dispatched.
...
- **Model / gating**: `ollama_chat_model` (still on the legacy Ollama-only path; PR 2.5b will route through the factory). Respects `llm_thinking_enabled`. Uses streaming when a token callback is provided, else direct.
...
- **Deflection rewrite (separate bulk op)**: `rewrite_all_diary_summaries()` (`POST /api/diary/scrub-deflections`) — for cleaning historical rows written before the prompt was tightened. One `ollama_chat_model` call per row (still legacy path, deferred to PR 2.5b) with `_REWRITE_DEFLECTION_SYSTEM_PROMPT`, asking the model to drop sentences that narrate the assistant's own failures while keeping everything else verbatim. Diary text is fenced as untrusted data (same fence used by the web tool). Preserves `ts_utc`; re-embeds updated rows best-effort. Empty-rewrite guard keeps the original if the model would have emptied the row. Fail-open at every layer (LLM call, write-back, embed). User-triggered from the Maintenance section in the diary sidebar.
- **Topic optimisation (separate bulk op)**: `optimise_diary_topics()` (`POST /api/diary/optimise-topics`) — collects all unique tags from `conversation_summaries`, makes one `ollama_chat_model` call (legacy path, deferred to PR 2.5b) with `_TOPIC_OPTIMISE_SYSTEM_PROMPT` to propose a normalised taxonomy (merge synonyms, split compound tags), then applies the mapping to every row that needs updating. Preserves `ts_utc`; re-embeds updated rows best-effort. User-triggered from the Maintenance section in the diary sidebar.
...
- **Model**: resolved chat model (factory-dispatched).
...
- **Model**: uses `picker_model` when passed through from `update_graph_from_dialogue` (daemon resolves it via `resolve_tool_router_model(cfg)` → small model when available). Falls back to the resolved chat model when no small model is configured. Factory-dispatched.
...
- **Model**: same `picker_model` chain as #11 (small router model when configured, falls back to the resolved chat model). Factory-dispatched. Temperature 0 — the task is rule-following classification.
...
- **Model / gating**: resolution chain `planner_model (override) → resolve_chat_model(cfg)`. Factory-dispatched. The planner tracks the active chat model so upgrading the chat model (via setup wizard or config) automatically upgrades plan quality — including provider switches to OpenAI-compatible.
...
- **Weather** ([src/jarvis/tools/builtin/weather.py](src/jarvis/tools/builtin/weather.py), ~line 60) — factory-dispatched. Place extractor model resolution: `tool_router_model → intent_judge_model → resolve_chat_model(cfg)` so small/warm models handle the parse without paging in the chat model. Parses location/time/unit from the query.
- **Nutrition log_meal** ([src/jarvis/tools/builtin/nutrition/log_meal.py](src/jarvis/tools/builtin/nutrition/log_meal.py), lines 48 & 136) — factory-dispatched. Both the nutrition extractor and the follow-up generator resolve via `resolve_chat_model(cfg)`. Extracts nutrients, confirms logging.
...
- Models: `llm_chat_model` (provider-aware, takes precedence) / `ollama_chat_model` (legacy alias, fallback), `intent_judge_model`, `tool_router_model`

...
- Response normalisation: `_normalise_response` lifts `choices[0].message` to top-level `message` so callers do not branch on provider. Everything else is copied unchanged, so `usage`, `stats` and `choices[0].finish_reason` stay reachable, and the thinking text rides along inside `message` as `reasoning_content` (LM Studio, llama.cpp) or `thinking` (Ollama). The intent judge reads both keys — its recovery and failure diagnostics depend on them surviving normalisation. Servers that already return Ollama-shaped responses pass through unchanged.
...
Backends fail soft for transient issues so the reply engine can degrade gracefully: timeouts and HTTP errors return `None` (or `[]` for `list_models`); HTTP 400 with `tools` set raises `ToolsNotSupportedError`; any other unexpected error is logged via `debug_log("...", "llm")` and returns `None`. The one exception is `requests.ConnectionError` (server unreachable), which `chat()` re-raises so callers like the intent judge can apply their own back-off — voice, for example, wants a 30s cooldown after a connection-refused error so it stops hammering an unresponsive Ollama between wake words. Timeouts discard any partially streamed text: `streaming()` returns `None` rather than a truncated prefix.

## Timeout budgets

Each `timeout_sec` is one wall-clock budget covering prefill plus decode for that single HTTP request; there is no cross-request total and no retry. Derive it as `prefill_seconds + max_tokens / tokens_per_sec`, then leave ~1.3x headroom. A call site's `max_tokens` therefore sets the smallest useful timeout: at 45 tok/s a 150-token cap needs ≥ 4 s of decode, a 300-token cap ≥ 7 s, and the judge's 1500-token thinking cap ≥ 34 s. The configured values in `config.py` (`llm_chat_timeout_sec` 180, `llm_tools_timeout_sec` 300, `llm_digest_timeout_sec` 12, `llm_embedding_timeout_sec` 60, `llm_profile_select_timeout_sec` 30, `intent_judge_timeout_sec` 15, `planner_timeout_sec` 10) follow that rule, and the `getattr` fallbacks at call sites mirror those defaults exactly so a partially-populated `Settings` cannot silently change the budget. All four model roles may resolve to one model name on a single-slot server; that slot serialises requests, and the queue wait counts against each request's own budget.

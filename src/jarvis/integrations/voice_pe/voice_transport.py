...
class AudioIngress:
    ...
    def push_frame(
        ...
        if key in self._selected:
            ...
            target = (
                AUDIO_CHANNEL_RAW
                if (self._config.audio_channel or "enhanced").lower() == "raw"
                else AUDIO_CHANNEL_ENHANCED
            )
            self._lock(
                key,
                target,
                f"fixed:{(self._config.audio_channel or 'enhanced').lower()}",
            )
            self._replay_from(key, self._first_voiced.get(key))

    def _deliver_now(self, frame: SatelliteAudioFrame, ch: int) -> None:
        """The lock is done: only the locked channel reaches the main VAD."""
    ...
    def dump_wavs(self, prefix: str, stream=None) -> dict:
        """Write ``channel-0-enhanced.wav`` and ``channel-1-raw.wav``."""

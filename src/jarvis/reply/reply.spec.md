...
6. Agentic Messages Loop with Dynamic Context
   ...
   - Evaluator contract:
     ...
     - Output: strict JSON `{"terminal": bool, "nudge": string, "reason": string, "tool_call": {"name": string, "arguments": object} | null}`. The `reason` field is a free-text log hint, never shown to the user. The `tool_call` field is optional; when present and the name is in the current allow-list (and is not `toolSearchTool`), the engine direct-executes the tool before the next chat turn. See `evaluator.spec.md` for the full semantics of the structured-tool-call path.
   ...
   - Structured-tool-call direct-exec: when `continue` also carries a `tool_call` whose `name` is in the allow-list (and is not `toolSearchTool`), the engine stashes it in `pending_tool_call`. At the top of the next loop iteration — before any chat LLM call — the engine synthesises an assistant message carrying the `tool_calls` payload, runs the tool via `run_tool_with_retries`, records the tool signature for duplicate suppression, and appends the tool result with the compound-query remainder hint (same as the model-emitted path). The textual nudge is cleared for that turn. This bypass exists because small chat models routinely ignore textual nudges; the evaluator already parsed the intent, so re-asking the model to parse it is lossy.
   - `cfg.evaluator_nudge_max` (default 2) caps TEXTUAL nudges per reply. A structured `tool_call` that will be direct-executed does NOT count against the cap (deterministic action, not a directive the model can ignore); one that fails the allow-list guard and falls back to the textual-nudge path DOES count. Once the cap is hit, the next textual-nudge `continue` is overridden to terminal to stop nudge ping-pong. Max-turn cap (`agentic_max_turns`) remains the hard backstop.

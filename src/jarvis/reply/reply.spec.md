...
6. Agentic Messages Loop with Dynamic Context
   ...
   - **Tool carry-over guard**: when the previous assistant turn invoked one or more tools and the current user query is a short follow-up (≤80 chars, char-length only — no language-specific keywords per CLAUDE.md), the previous turn's tool names are unioned back into the allow-list before the planner schema is generated. Compensates for small routers that misroute follow-ups (field trace 2026-05-03: turn 1 invoked `getWeather`, turn 2 was "I'm in London", router picked `webSearch`, planner web-searched "weather in london tomorrow", Wikipedia fallback returned "Edge of Tomorrow" and the assistant parrotted the film summary as the weather answer). The carry-over walks `recent_messages` from the end backwards and collects tool names from both calling protocols (native: `assistant.tool_calls[*].function.name`; text-tool fallback: `user.tool_name`), stopping at the first genuine user message. The augmentation is an engine-side per-turn overlay: the router cache stores only the raw router output, so identical-query replays in future turns are unaffected. When carry-over fires, `_selection_source` becomes `<strategy>+carryover` (or `<strategy>+plan+carryover`) so the printed `🔧 Tools` log line stays honest. The 80-char cutoff is the same shape as the recall gate's content-word heuristic — long queries are treated as deliberate topic changes where the router pick stands on its own.

...
2. **Provider-aware Ollama gating**: The Ollama server-start and model-verification steps run only when a local provider actually uses Ollama (`_ollama_runtime_flags` in `app.py`). A pure OpenAI-compatible setup (chat and embeddings both remote) skips them entirely. When only embeddings run on Ollama, the server is still started but the chat-model checks are skipped (the chat model isn't an Ollama model). `should_show_setup_wizard()` likewise returns False for an OpenAI-compatible user.
